...
all_datagen = HSI_datagen(all_kiwi_NIR, 'header_file', 'data_file', {'name': 'ripeness_state_y', 'type': int}, batch_size=9, target_size=(200, 200), data_dir=data_dir,
...
#  create and run gan 
gan = HSDWCGAN(x=200, y=200, c=252, num_classes=3, ld=100, optimiser=adam, loss=['binary_crossentropy'], metrics=['accuracy'], use_HS_CNN=True, working_dir=working_dir + "gan_model2/")

# 
gan.train(epochs=10000, batch_size=9, sample_interval=100, save_interval=100, save_dir=working_dir + "gan_model2/", datagen=all_datagen)