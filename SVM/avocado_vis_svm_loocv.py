...
results, all_pred = SVC_LOOCV(all_avocado_VIS, augment=False, balance=False, verbose=1)

accuracy = np.mean(results)

print(f"Accuracy: {accuracy}")
np.save(os.path.join(working_dir, "SVM/avocado_vis_svm_loocv.npy"), results)
np.save(os.path.join(working_dir, "SVM/avocado_vis_svm_loocv_preds.npy"), all_pred)