...
def LOOCV(dataset, augment = False, augConfig = None, balance = False, verbose=0):
    ...
    for index, row in dataset.iterrows():
        ...
        temp_datagen = HSI_datagen(temp_df, 'header_file', 'data_file', {'name': 'ripeness_state_y', 'type': int}, batch_size=len(all_kiwi_VIS)-1, target_size=(64, 64), data_dir=data_dir,
...
results, all_pred = LOOCV(all_kiwi_VIS, augment=False, balance=False, verbose=1)

accuracy = np.mean(results)

print(f"Accuracy: {accuracy}")
np.save(os.path.join(working_dir, "DT/kiwi_vis_dt_loocv.npy"), results)
np.save(os.path.join(working_dir, "DT/kiwi_vis_dt_loocv_preds.npy"), all_pred)