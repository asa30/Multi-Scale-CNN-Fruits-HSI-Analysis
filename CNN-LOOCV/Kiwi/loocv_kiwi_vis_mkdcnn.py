...
# build the model
kiwi_vis_model = build_MKDCNN((64, 64, 224))

kiwi_vis_model.compile(optimizer=RMSprop(learning_rate=1e-2), loss='sparse_categorical_crossentropy')

kiwi_vis_loocv_results = LOOCV(kiwi_vis_model, all_kiwi_VIS, 12, epochs=40, save_path="kiwi_vis_mkdcnn_loocv", verbose=1)

print(f"Accuracy of LOOCV for MKDCNN model tested on Kiwi VIS: {np.mean(kiwi_vis_loocv_results)}")

# save the results
np.save(os.path.join(working_dir, "kiwi_vis_mkdcnn_loocv_results.npy"), kiwi_vis_loocv_results)
