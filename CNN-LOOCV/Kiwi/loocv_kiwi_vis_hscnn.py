...
kiwi_vis_loocv_results = LOOCV(kiwi_vis_model, all_kiwi_VIS, 12, epochs=40, save_path="kiwi_vis_hs_cnn_loocv", verbose=1)

print(f"Accuracy of LOOCV for HSCNN model tested on Kiwi VIS: {np.mean(kiwi_vis_loocv_results)}")

# save the results
np.save(os.path.join(working_dir, "kiwi_vis_hs_cnn_loocv_results.npy"), kiwi_vis_loocv_results)
