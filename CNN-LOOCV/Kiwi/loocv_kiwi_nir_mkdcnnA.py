...
# build the model
kiwi_nir_model = build_MKDCNN_Attention((64, 64, 252))

kiwi_nir_model.compile(optimizer=RMSprop(learning_rate=1e-2), loss='sparse_categorical_crossentropy')

kiwi_nir_loocv_results = LOOCV(kiwi_nir_model, all_kiwi_NIR, 12, epochs=40, save_path="kiwi_nir_mkdcnnA_loocv", verbose=1)

print(f"Accuracy of LOOCV for MKDCNN with attention model tested on Kiwi NIR: {np.mean(kiwi_nir_loocv_results)}")

# save the results
np.save(os.path.join(working_dir, "kiwi_nir_mkdcnnA_loocv_results.npy"), kiwi_nir_loocv_results)
