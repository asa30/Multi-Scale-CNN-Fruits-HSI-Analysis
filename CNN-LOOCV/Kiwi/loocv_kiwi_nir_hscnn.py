...
kiwi_nir_loocv_results = LOOCV(kiwi_nir_model, all_kiwi_NIR, 12, epochs=40, save_path="kiwi_nir_hs_cnn_loocv", verbose=1)

print(f"Accuracy of LOOCV for HSCNN model tested on Kiwi NIR: {np.mean(kiwi_nir_loocv_results)}")

# save the results
np.save(os.path.join(working_dir, "kiwi_nir_hs_cnn_loocv_results.npy"), kiwi_nir_loocv_results)
