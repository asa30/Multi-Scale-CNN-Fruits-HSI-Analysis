...
avocado_vis_model = build_MSDCNN((64, 64, 224))

avocado_vis_model.compile(optimizer=RMSprop(learning_rate=1e-2), loss='sparse_categorical_crossentropy')

avocado_vis_loocv_results = LOOCV(avocado_vis_model, all_avocado_VIS, 12, 40, save_path="avocado_vis_msdcnn_loocv", verbose=1)

print(f"Accuracy of LOOCV for MSDCNN model tested on Avocado VIS: {np.mean(avocado_vis_loocv_results)}")
# save the results
np.save(os.path.join(working_dir, "avocado_vis_msdcnn_loocv_results.npy"), avocado_vis_loocv_results)