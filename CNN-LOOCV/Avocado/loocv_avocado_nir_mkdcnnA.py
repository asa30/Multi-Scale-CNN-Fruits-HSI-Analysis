...
avocado_nir_model = build_MKDCNN_Attention((64, 64, 252))

avocado_nir_model.compile(optimizer=RMSprop(learning_rate=1e-2), loss='sparse_categorical_crossentropy')

avocado_nir_loocv_results = LOOCV(avocado_nir_model, all_avocado_NIR, 12, 40, save_path="avocado_nir_mkdcnnA_loocv", verbose=1)

print(f"Accuracy of LOOCV for MKDCNN with attention model tested on Avocado NIR: {np.mean(avocado_nir_loocv_results)}")
# save the results
np.save(os.path.join(working_dir, "avocado_nir_mkdcnnA_loocv_results.npy"), avocado_nir_loocv_results)