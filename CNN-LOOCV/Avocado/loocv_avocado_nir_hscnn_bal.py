...
avocado_nir_loocv_results = LOOCV(avocado_nir_model, all_avocado_NIR, 12, 40, augment=True, augConfig=augConfig, balance = True, save_path="avocado_nir_hs_cnn_loocv_balanced", verbose=1)

print(f"Accuracy of LOOCV for HSCNN model tested on Avocado NIR: {np.mean(avocado_nir_loocv_results)}")
# save the results
np.save(os.path.join(working_dir, "avocado_nir_hs_cnn_loocv_balanced_results.npy"), avocado_nir_loocv_results)